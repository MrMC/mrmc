...
#include "../reader/ifo_types.h" /* vm_cmd_t */
...
uint32_t vm_getbits(command_t *command, int32_t start, int32_t count) {
  ...
  if ( ((start - count) < -1) ||
       ...
       (count < 0) ||
       (start < 0) ) {
    DVD_DEBUG(DBG_CRIT, "libdvdnav: Bad call to vm_getbits. Parameter out of range\n");
    abort();
  }
  /* all ones, please */
  bit_mask = ~bit_mask;
...
static uint16_t get_GPRM(registers_t* registers, uint8_t reg) {
  if (registers->GPRM_mode[reg] & 0x01) {
    ...
    uint16_t result;
    /* Counter mode */
    /* DVD_DEBUG(DBG_CRIT, "libdvdnav: Getting counter %d\n",reg);*/
...
static void set_GPRM(registers_t* registers, uint8_t reg, uint16_t value) {
  if (registers->GPRM_mode[reg] & 0x01) {
    ...
    /* Counter mode */
    /* DVD_DEBUG(DBG_CRIT, "libdvdnav: Setting counter %d\n",reg); */
    gettimeofday(&current_time, NULL);
...
static uint16_t eval_reg(command_t* command, uint8_t reg) {
  if(reg & 0x80) {
    if ((reg & 0x1f) == 20) {
      DVD_DEBUG(DBG_CRIT, "libdvdnav: Suspected RCE Region Protection!!!\n");
    }
...
static int32_t eval_compare(uint8_t operation, uint16_t data1, uint16_t data2) {
  ...
  DVD_DEBUG(DBG_CRIT, "libdvdnav: eval_compare: Invalid comparison code\n");
  return 0;
}
...
static int32_t eval_command(uint8_t *bytes, registers_t* registers, link_t *return_values) {
  ...
  switch(vm_getbits(&command, 63, 3)) { /* three first old_bits */
    case 0: /*  Special instructions */
      ...
      if(res == -1) {
        DVD_DEBUG(DBG_CRIT, "libdvdnav: Unknown Instruction!\n");
        assert(0);
      }
      break;
    ...
    default: /* Unknown command type */
      DVD_DEBUG(DBG_CRIT, "libdvdnav: WARNING: Unknown Command Type=%x\n", vm_getbits(&command, 63, 3));
  }
  /*  Check if there are bits not yet examined */

  if(command.instruction & ~ command.examined) {
    DVD_DEBUG(DBG_CRIT, "libdvdnav: decoder.c: [WARNING, unknown bits:");
    DVD_DEBUG(DBG_CRIT, " %08"PRIx64, (command.instruction & ~ command.examined) );
    DVD_DEBUG(DBG_CRIT, "]\n");
  }
...
#ifdef TRACE
  /*  DEBUG */
  DVD_DEBUG(DBG_CRIT, "libdvdnav: Registers before transaction\n");
  vm_print_registers( registers );
  DVD_DEBUG(DBG_CRIT, "libdvdnav: Full list of commands to execute\n");
  for(i = 0; i < num_commands; i++)
    vm_print_cmd(i, &commands[i]);
  DVD_DEBUG(DBG_CRIT, "libdvdnav: --------------------------------------------\n");
  DVD_DEBUG(DBG_CRIT, "libdvdnav: Single stepping commands\n");
#endif
...
#ifdef TRACE
      DVD_DEBUG(DBG_CRIT, "libdvdnav: Registers after transaction\n");
      vm_print_registers( registers );
      DVD_DEBUG(DBG_CRIT, "libdvdnav: eval: Doing Link/Jump/Call\n");
#endif
      return 1;
    }
...
void vm_print_link(link_t value) {
  ...
  case LinkTailPGC:
  case LinkRSM:
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s (button %d)\n", cmd, value.data1);
    break;
  case LinkPGCN:
  case JumpTT:
  case JumpVTS_TT:
  ...
  case JumpSS_VMGM_PGC:
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s %d\n", cmd, value.data1);
    break;
  case LinkPTTN:
  case LinkPGN:
  case LinkCN:
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s %d (button %d)\n", cmd, value.data1, value.data2);
    break;
  case Exit:
  case JumpSS_FP:
  case PlayThis: /*  Humm.. should we have this at all.. */
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s\n", cmd);
    break;
  case JumpVTS_PTT:
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s %d:%d\n", cmd, value.data1, value.data2);
    break;
  case JumpSS_VTSM:
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s vts %d title %d menu %d\n",
            cmd, value.data1, value.data2, value.data3);
    break;
  case CallSS_FP:
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s resume cell %d\n", cmd, value.data1);
    break;
  case CallSS_VMGM_MENU: /*  == 2 -> Title Menu */
  case CallSS_VTSM:
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s %d resume cell %d\n", cmd, value.data1, value.data2);
    break;
  case CallSS_VMGM_PGC:
    DVD_DEBUG(DBG_CRIT, "libdvdnav: %s %d resume cell %d\n", cmd, value.data1, value.data2);
    break;
...
void vm_print_registers( registers_t *registers ) {
  ...
  DVD_DEBUG(DBG_CRIT, "libdvdnav:    #   ");
  for(i = 0; i < 24; i++)
    DVD_DEBUG(DBG_CRIT, " %2d |", i);
  DVD_DEBUG(DBG_CRIT, "\nlibdvdnav: SRPMS: ");
  for(i = 0; i < 24; i++)
    DVD_DEBUG(DBG_CRIT, "%04x|", registers->SPRM[i]);
  DVD_DEBUG(DBG_CRIT, "\nlibdvdnav: GRPMS: ");
  for(i = 0; i < 16; i++)
    DVD_DEBUG(DBG_CRIT, "%04x|", get_GPRM(registers, i) );
  DVD_DEBUG(DBG_CRIT, "\nlibdvdnav: Gmode: ");
  for(i = 0; i < 16; i++)
    DVD_DEBUG(DBG_CRIT, "%04x|", registers->GPRM_mode[i]);
  DVD_DEBUG(DBG_CRIT, "\nlibdvdnav: Gtime: ");
  for(i = 0; i < 16; i++)
    DVD_DEBUG(DBG_CRIT, "%04lx|", registers->GPRM_time[i].tv_sec & 0xffff);
  DVD_DEBUG(DBG_CRIT, "\n");
}

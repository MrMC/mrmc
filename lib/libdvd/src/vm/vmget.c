...
int vm_get_current_title_part(vm_t *vm, int *title_result, int *part_result) {
  ...
  if (!found) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: chapter NOT FOUND!\n");
    return 0;
  }
...
#ifdef TRACE
  if (title) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ************ this chapter FOUND!\n");
    DVD_DEBUG(DBG_DVD, "libdvdnav: VTS_PTT_SRPT - Title %3i part %3i: PGC: %3i PG: %3i\n",
             title, part,
...
int vm_get_video_aspect(vm_t *vm) {
  ...
  if(aspect != 0 && aspect != 3) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: display aspect ratio is unexpected: %d!\n", aspect);
    return -1;
  }

...
void vm_position_print(vm_t *vm, vm_position_t *position) {
  DVD_DEBUG(DBG_DVD, "libdvdnav: But=%x Spu=%x Aud=%x Ang=%x Hop=%x vts=%x dom=%x cell=%x cell_restart=%x cell_start=%x still=%x block=%x\n",
  position->button,
...
static void vm_print_current_domain_state(vm_t *vm) {
  ...
  DVD_DEBUG(DBG_DVD, "libdvdnav: %s Domain: VTS:%d PGC:%d PG:%u CELL:%u BLOCK:%u VTS_TTN:%u TTN:%u TT_PGCN:%u\n",
                   domain,
                   vm->state.vtsN,
                   get_PGCN(vm),
                   vm->state.pgN,
...
static int dvd_read_name(char *name, char *serial, const char *device) {
  ...
  /* Read DVD name */
  if (device == NULL) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: Device name string NULL\n");
    goto fail;
  }
  if ((fd = open(device, O_RDONLY)) == -1) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: Unable to open device file %s.\n", device);
    goto fail;
  }
  ...
  if( off != ( 32 * (int64_t) DVD_VIDEO_LB_LEN ) ) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: Can't seek to block %u\n", 32 );
    goto fail;
  }

  if ((read_size = read( fd, data, DVD_VIDEO_LB_LEN )) == -1) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: Can't read name block. Probably not a DVD-ROM device.\n");
    goto fail;
  }
  ...
  DVD_DEBUG(DBG_DVD, "libdvdnav: DVD Title: ");
  for(i=25; i < 73; i++ ) {
    if(data[i] == 0) break;
    if((data[i] > 32) && (data[i] < 127)) {
      DVD_DEBUG(DBG_DVD, "%c", data[i]);
    } else {
      DVD_DEBUG(DBG_DVD, " ");
    }
  }
...
int ifoOpenNewVTSI(vm_t *vm, dvd_reader_t *dvd, int vtsN) {
  ...
  if(vm->vtsi == NULL) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ifoOpenVTSI failed\n");
    return 0;
  }
  if(!ifoRead_VTS_PTT_SRPT(vm->vtsi)) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ifoRead_VTS_PTT_SRPT failed\n");
    return 0;
  }
  if(!ifoRead_PGCIT(vm->vtsi)) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ifoRead_PGCIT failed\n");
    return 0;
  }
  if(!ifoRead_PGCI_UT(vm->vtsi)) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ifoRead_PGCI_UT failed\n");
    return 0;
  }
  if(!ifoRead_VOBU_ADMAP(vm->vtsi)) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ifoRead_VOBU_ADMAP vtsi failed\n");
    return 0;
  }
  if(!ifoRead_TITLE_VOBU_ADMAP(vm->vtsi)) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ifoRead_TITLE_VOBU_ADMAP vtsi failed\n");
    return 0;
  }
  if (!ifoRead_VTS_TMAPT(vm->vtsi)) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ifoRead_VTS_TMAPT vtsi failed\n");
    return 0;    
  }
  if (!ifoRead_TITLE_C_ADT(vm->vtsi)) {
    DVD_DEBUG(DBG_DVD, "libdvdnav: ifoRead_TITLE_C_ADT vtsi failed\n");
    return 0;
  }
...
int vm_reset(vm_t *vm, const char *dvdroot) {
  ...
  if (!vm->dvd) {
    ...
    if(!vm->dvd) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: failed to open/read the DVD\n");
      return 0;
    }
    vm->vmgi = ifoOpenVMGI(vm->dvd);
    if(!vm->vmgi) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: failed to read VIDEO_TS.IFO\n");
      return 0;
    }
    if(!ifoRead_FP_PGC(vm->vmgi)) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: ifoRead_FP_PGC failed\n");
      return 0;
    }
    if(!ifoRead_TT_SRPT(vm->vmgi)) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: ifoRead_TT_SRPT failed\n");
      return 0;
    }
    if(!ifoRead_PGCI_UT(vm->vmgi)) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: ifoRead_PGCI_UT failed\n");
      return 0;
    }
    if(!ifoRead_PTL_MAIT(vm->vmgi)) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: ifoRead_PTL_MAIT failed\n");
      /* return 0; Not really used for now.. */
    }
    if(!ifoRead_VTS_ATRT(vm->vmgi)) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: ifoRead_VTS_ATRT failed\n");
      /* return 0; Not really used for now.. */
    }
    if(!ifoRead_VOBU_ADMAP(vm->vmgi)) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: ifoRead_VOBU_ADMAP vgmi failed\n");
      /* return 0; Not really used for now.. */
    }
    /* ifoRead_TXTDT_MGI(vmgi); Not implemented yet */
    if(dvd_read_name(vm->dvd_name, vm->dvd_serial, dvdroot) != 1) {
      DVD_DEBUG(DBG_DVD, "libdvdnav: vm: dvd_read_name failed\n");
    }
#ifdef _XBMC
    ...
    DVD_DEBUG(DBG_DVD, "libdvdnav: vm: DVD Title: %s\n", vm->dvd_name);
#endif
  }
  if (vm->vmgi) {
    int i, mask;
    DVD_DEBUG(DBG_DVD, "libdvdnav: DVD disk reports itself with Region mask 0x%08x. Regions:",
      vm->vmgi->vmgi_mat->vmg_category);
    for (i = 1, mask = 1; i <= 8; i++, mask <<= 1)
      if (((vm->vmgi->vmgi_mat->vmg_category >> 16) & mask) == 0)
        DVD_DEBUG(DBG_DVD, " %d", i);
    DVD_DEBUG(DBG_DVD, "\n");
  }
  return 1;
}
  ...
  while(link_values.command != PlayThis) {
...
#ifdef TRACE
    DVD_DEBUG(DBG_DVD, "libdvdnav: Before printout starts:\n");
    vm_print_link(link_values);
    DVD_DEBUG(DBG_DVD, "libdvdnav: Link values %i %i %i %i\n", link_values.command,
            link_values.data1, link_values.data2, link_values.data3);
    vm_print_current_domain_state(vm);
    DVD_DEBUG(DBG_DVD, "libdvdnav: Before printout ends.\n");
#endif
    ...
    case LinkRSM:
        ...
        if (!vm->state.rsm_vtsN) {
          DVD_DEBUG(DBG_DVD, "libdvdnav: trying to resume without any resume info set\n");
          link_values.command = Exit;
          break;
        }
...
#ifdef TRACE
    DVD_DEBUG(DBG_DVD, "libdvdnav: After printout starts:\n");
    vm_print_current_domain_state(vm);
    DVD_DEBUG(DBG_DVD, "libdvdnav: After printout ends.\n");
#endif

...
#ifdef TRACE
  DVD_DEBUG(DBG_DVD, "libdvdnav: play_PGC:");
  if((vm->state).domain != DVD_DOMAIN_FirstPlay) {
    DVD_DEBUG(DBG_DVD, " (vm->state).pgcN (%i)\n", get_PGCN(vm));
  } else {
    DVD_DEBUG(DBG_DVD, " first_play_pgc\n");
  }
#endif
...
#ifdef TRACE
      DVD_DEBUG(DBG_DVD, "libdvdnav: PGC pre commands didn't do a Jump, Link or Call\n");
#endif
    }
  }
  return play_PG(vm);
...
#ifdef TRACE
  DVD_DEBUG(DBG_DVD, "libdvdnav: play_PGC_PG:");
  if((vm->state).domain != DVD_DOMAIN_FirstPlay) {
    DVD_DEBUG(DBG_DVD, " (vm->state).pgcN (%i)\n", get_PGCN(vm));
  } else {
    DVD_DEBUG(DBG_DVD, " first_play_pgc\n");
...
#ifdef TRACE
  DVD_DEBUG(DBG_DVD, "libdvdnav: play_PGC_post:\n");
#endif
...
#ifdef TRACE
  DVD_DEBUG(DBG_DVD, "libdvdnav: ** Fell of the end of the pgc, continuing in NextPGC\n");
#endif
...
#ifdef TRACE
  DVD_DEBUG(DBG_DVD, "libdvdnav: play_PG: (vm->state).pgN (%i)\n", (vm->state).pgN);
#endif

  assert((vm->state).pgN > 0);
  if((vm->state).pgN > (vm->state).pgc->nr_of_programs) {
#ifdef TRACE
    DVD_DEBUG(DBG_DVD, "libdvdnav: play_PG: (vm->state).pgN (%i) > pgc->nr_of_programs (%i)\n",
...
link_t play_Cell(vm_t *vm) {
...
#ifdef TRACE
  DVD_DEBUG(DBG_DVD, "libdvdnav: play_Cell: (vm->state).cellN (%i)\n", (vm->state).cellN);
#endif

  assert((vm->state).cellN > 0);
  if((vm->state).cellN > (vm->state).pgc->nr_of_cells) {
#ifdef TRACE
    DVD_DEBUG(DBG_DVD, "libdvdnav: (vm->state).cellN (%i) > pgc->nr_of_cells (%i)\n",
  ...
  case 1: /*  The first cell in the block */
    ...
    case 1: /*  Angle block */
      ...
      if (!((vm->state).cellN <= (vm->state).pgc->nr_of_cells) ||
        ...
        DVD_DEBUG(DBG_DVD, "libdvdnav: Invalid angle block\n");
        (vm->state).cellN -= (vm->state).AGL_REG - 1;
      }
#endif
      break;
    ...
    case 3: /*  ?? */
    default:
      DVD_DEBUG(DBG_DVD, "libdvdnav: Invalid? Cell block_mode (%d), block_type (%d)\n",
  ...
  default:
    DVD_DEBUG(DBG_DVD, "libdvdnav: Cell is in block but did not enter at first cell!\n");
  }
  ...
  (vm->state).blockN = 0;
#ifdef TRACE
  DVD_DEBUG(DBG_DVD, "libdvdnav: Cell should restart here\n");
#endif
  return play_this;
}
...
link_t play_Cell_post(vm_t *vm) {
...
#ifdef TRACE
  DVD_DEBUG(DBG_DVD, "libdvdnav: play_Cell_post: (vm->state).cellN (%i)\n", (vm->state).cellN);
#endif
...
#ifdef TRACE
      DVD_DEBUG(DBG_DVD, "libdvdnav: Cell command present, executing\n");
#endif
...
#ifdef TRACE
        DVD_DEBUG(DBG_DVD, "libdvdnav: Cell command didn't do a Jump, Link or Call\n");
#endif
      }
    } else {
#ifdef TRACE
      DVD_DEBUG(DBG_DVD, "libdvdnav: Invalid Cell command\n");
#endif
    }
  }
  ...
  if(!set_PGN(vm)) {
#ifdef TRACE
    DVD_DEBUG(DBG_DVD, "libdvdnav: last cell in this PGC\n");
#endif
    return play_PGC_post(vm);
  }
  return play_Cell(vm);

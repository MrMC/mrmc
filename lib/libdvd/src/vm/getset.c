...
#ifdef TRACE
    DVD_DEBUG(DBG_CRIT, "libdvdnav:  ** No such pgcN = %d\n", pgcN);
#endif
    return 0;
  }
...
int get_ID(vm_t *vm, int id) {
  ...
  pgcit = get_PGCIT(vm);
  if(pgcit == NULL) {
    DVD_DEBUG(DBG_CRIT, "libdvdnav: PGCIT null!\n");
    return 0;
  }
#ifdef TRACE
  DVD_DEBUG(DBG_CRIT, "libdvdnav: ** Searching for menu (0x%x) entry PGC\n", id);
#endif
  ...
  for(i = 0; i < pgcit->nr_of_pgci_srp; i++) {
    if( (pgcit->pgci_srp[i].entry_id) == id) {
      pgcN = i + 1;
#ifdef TRACE
      DVD_DEBUG(DBG_CRIT, "libdvdnav: Found menu.\n");
#endif
      return pgcN;
    }
  }
#ifdef TRACE
  DVD_DEBUG(DBG_CRIT, "libdvdnav: ** No such id/menu (0x%02x) entry PGC\n", id & 0x7f);
  for(i = 0; i < pgcit->nr_of_pgci_srp; i++) {
    if ( (pgcit->pgci_srp[i].entry_id & 0x80) == 0x80) {
      DVD_DEBUG(DBG_CRIT, "libdvdnav: Available menus: 0x%x\n",
...
int get_PGCN(vm_t *vm) {
  ...
  DVD_DEBUG(DBG_CRIT, "libdvdnav: get_PGCN failed. Was trying to find pgcN in domain %d\n",
         (vm->state).domain);
  return 0; /*  error */
}
...
pgcit_t* get_MENU_PGCIT(vm_t *vm, ifo_handle_t *h, uint16_t lang) {
  ...
  if(h == NULL || h->pgci_ut == NULL) {
    DVD_DEBUG(DBG_CRIT, "libdvdnav: *** pgci_ut handle is NULL ***\n");
    return NULL; /*  error? */
  }
  ...
  if(i == h->pgci_ut->nr_of_lus) {
    DVD_DEBUG(DBG_CRIT, "libdvdnav: Language '%c%c' not found, using '%c%c' instead\n",
            (char)(lang >> 8), (char)(lang & 0xff),
             (char)(h->pgci_ut->lu[0].lang_code >> 8),
            (char)(h->pgci_ut->lu[0].lang_code & 0xff));
    DVD_DEBUG(DBG_CRIT, "libdvdnav: Menu Languages available: ");
    for(i = 0; i < h->pgci_ut->nr_of_lus; i++) {
      DVD_DEBUG(DBG_CRIT, "%c%c ",

...
static dvd_reader_t *DVDOpenImageFile(const char *location)
{
  ...
  if(!dev) {
    DVD_DEBUG(DBG_CRIT, "Can't open %s for reading\n", location);
    return NULL;
  }
...
dvd_reader_t* DVDOpen(const char *ppath )
{
  ...
  if (directory_exists(path)) {
    ...
    if (!dev_name) {
      DVD_DEBUG(DBG_CRIT, "libdvdread: Couldn't find device name.\n" );
    } else if( !auth_drive ) {
      DVD_DEBUG(DBG_CRIT, "libdvdread: Device %s inaccessible, "
    ...
    if (ret < 0) {
      ...
      if (strchr(path,':')) {
        ret_val = DVDOpenImageFile(path);
        free(path);
        return ret_val;
      }

      /* If we can't stat the file, give up */
      DVD_DEBUG(DBG_CRIT, "libdvdread: Can't stat %s\n", path );
      perror("");
      goto DVDOpen_error;
    }
...
DVDOpen_error:
  ...
  DVD_DEBUG(DBG_CRIT, "libdvdread: Could not open %s\n", path);
  free(path);
  free(path_copy);
#if !defined(WIN32) && !defined(_XBMC)
  if (cdir >= 0)
                                   ...
                                   int do_cache )
{
  ...
  if( !start ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread:DVDOpenFileUDF:DVDFindFile %s failed\n", filename );
    return NULL;
  }

  dvd_file = calloc( 1, sizeof( dvd_file_t ) );
  if( !dvd_file ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread:DVDOpenFileUDF:malloc failed\n" );
    return NULL;
  }
  dvd_file->dvd = dvd;
...
static dvd_file_t *DVDOpenFilePath( dvd_reader_t *dvd, const char *filename )
{
  ...
  if( !findDVDFile( dvd, filename, full_path ) ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread:DVDOpenFilePath:findDVDFile %s failed\n", filename );
    return NULL;
  }

  dev = dvdinput_open( full_path);
  if( !dev ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread:DVDOpenFilePath:dvdinput_open %s failed\n", full_path );
    return NULL;
  }

  dvd_file = calloc( 1, sizeof( dvd_file_t ) );
  if( !dvd_file ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread:DVDOpenFilePath:dvd_file malloc failed\n" );
    dvdinput_close(dev);
    return NULL;
  }
  dvd_file->dvd = dvd;
  dvd_file->title_sizes[0] = file_size(dev->fd) / DVD_VIDEO_LB_LEN;
...
static dvd_file_t *DVDOpenVOBPath( dvd_reader_t *dvd, int title, int menu )
{
  ...
  if (menu) {
    ...
    for( i = 0; i < TITLES_MAX; ++i ) {
      ...
      dvd_file->title_sizes[i] = file_size(dev->fd) / DVD_VIDEO_LB_LEN;
      dvd_file->title_devs[i] = dev;
                         ...
                         dvd_read_domain_t domain )
{
  ...
  case DVD_READ_TITLE_VOBS:
    ...
    break;
  default:
    DVD_DEBUG(DBG_CRIT, "libdvdread: Invalid domain for file open.\n" );
    return NULL;
  }
                 ...
                 dvd_read_domain_t domain, dvd_stat_t *statbuf )
{
  ...
  case DVD_READ_TITLE_VOBS:
    ...
    break;
  default:
    DVD_DEBUG(DBG_CRIT, "libdvdread: Invalid domain for file stat.\n" );
    errno = EINVAL;
    return -1;
  }
  ...
  if( dvd->isImageFile ) {
    ...
    if (findDVDFile(dvd, filename, full_path)) {
      DVD_FILEIO_H *fd;
      fd = file_open(full_path, "rb");
      if (!fd)
        DVD_DEBUG(DBG_CRIT, "libdvdread: Can't stat() %s.\n", filename );
      else {
        statbuf->size = file_size(fd);
        statbuf->nr_parts = 1;
                      ...
                      int encrypted )
{
  ...
  if( !device->dev ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread: Fatal error in block read.\n" );
    return 0;
  }

  ret = dvdinput_seek( device->dev, (int) lb_number );
  if( ret != (int) lb_number ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread: Can't seek to block %u\n", lb_number );
    return 0;
  }
  ...
  for( i = 0; i < TITLES_MAX; ++i ) {
    ...
    if( offset < dvd_file->title_sizes[ i ] ) {
      if( ( offset + block_count ) <= dvd_file->title_sizes[ i ] ) {
        ...
        if( off < 0 || off != 0 ) {
          DVD_DEBUG(DBG_CRIT, "libdvdread: Can't seek to block %d\n",
                   0 );
          return off < 0 ? off : 0;
        }
...
int DVDFileSeekForce(dvd_file_t *dvd_file, int offset, int force_size)
{
  ...
  if( dvd_file->dvd->isImageFile ) {
    ...
    if( dvd_file->filesize < force_size ) {
      ...
      DVD_DEBUG(DBG_CRIT, "libdvdread: Ignored size of file indicated in UDF.\n");
    }
  }
...
ssize_t DVDReadBytes( dvd_file_t *dvd_file, void *data, size_t byte_size )
{
  ...
  if( !secbuf_base ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread: Can't allocate memory "
             "for file read!\n" );
    return 0;
  }
...
int DVDDiscID( dvd_reader_t *dvd, unsigned char *discid )
{
  ...
  if( !vmg_ifo ) {
    DVD_DEBUG(DBG_CRIT, "libdvdread: DVDDiscId, failed to "
      "open VMG IFO!\n" );
    return -1;
  }
  ...
  for( title = 0; title < title_sets; title++ ) {
    ...
    if( dvd_file != NULL ) {
      ...
      if (bytes_read != file_size) {
          DVD_DEBUG(DBG_CRIT, "libdvdread: DVDDiscId read returned %zd bytes"
      ...
      DVDAddMD5(&ctx, buffer, file_size);
                      ...
                      unsigned char *volsetid, unsigned int volsetid_size )
{
  ...
  if (buffer_base == NULL) {
    DVD_DEBUG(DBG_CRIT, "libdvdread: DVDISOVolumeInfo, failed to "
             "allocate memory for file read!\n" );
    return -1;
  }

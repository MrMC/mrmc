...
uint32_t            debug_mask = (uint32_t)-1; /* set all bits to make sure dvd_debug() is called for initialization */
static DVD_LOG_FUNC  log_func   = NULL;

void dvd_set_debug_handler(DVD_LOG_FUNC f)
{
    log_func = f;
}
...
void dvd_debug(const char *file, int line, uint32_t mask, const char *format, ...)
{
    ...
    if (!debug_init) {
        ...
        if ((env = getenv("DVD_DEBUG_MASK")))
            debug_mask = strtol(env, NULL, 0);

        // Send DEBUG to file?
        if ((env = getenv("DVD_DEBUG_FILE"))) {

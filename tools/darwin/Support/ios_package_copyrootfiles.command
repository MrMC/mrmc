...
if [ "$ACTION" == build ] || [ "$ACTION" == install ]; then
  ...
  ADDONSYNC="rsync -aq ${PLATFORM} ${BUILDSRC} ${BUILDDBG} --exclude addons/lib --exclude addons/share --exclude addons/skin.mrmc --exclude addons/skin.re-touched --exclude *changelog.* --exclude *library.*/*.h --exclude *library.*/*.cpp --exclude *xml.in"

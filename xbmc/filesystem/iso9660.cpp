...
struct iso_dirtree *iso9660::ReadRecursiveDirFromSector( uint32_t sector, const char *path )
{
  ...
  uint16_t wSectorSize = from_723(m_info.iso.logical_block_size);
...
//******************************************************************************************************************
std::string iso9660::GetThinText(uint8_t* strTxt, int iLen )
{
...
bool iso9660::ReadSectorFromCache(iso9660::isofile* pContext, uint32_t sector, uint8_t** ppBuffer)
{
  ...
  {
    // Just retrieve it
    uint32_t SectorInCircBuff = (sector - StartSectorInCircBuff) +

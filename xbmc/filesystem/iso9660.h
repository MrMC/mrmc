...
struct iso9660_VolumeDescriptor
{
  ...
  uint16_t wVolumeSetSizeLE;       //120-121
  uint16_t wVolumeSetSizeBE;       //122-123
  uint16_t wVolumeSequenceNumberLE;   //124-125
  uint16_t wVolumeSequenceNumberBE;   //126-127
  ...
  uint32_t wSecondPathTableStartSectorLE; //144-147
  uint32_t wFirstPathTableStartSectorBE; //148-151
  uint32_t wSecondPathTableStartSectorBE; //152-155
...
struct  iso9660_Datetime {
  ...
  uint8_t month;  /* Has value in range 1..12. Note starts
...
#define Flag_NotExist  0x01     /* 1-file not exists */
 ...
 #define Flag_Multi   0x07     /* 0-final Directory Record for the file */
  ...
  uint8_t ucRecordLength;      //0      the number of uint8_ts in the record (which must be even)
  uint8_t ucExtendAttributeSectors; //1      [number of sectors in extended attribute record]
  ...
  uint8_t byFlags;         //25     flags
  uint8_t UnitSize;         //26     file unit size for an interleaved file
  uint8_t InterleaveGapSize;    //27     interleave gap size for an interleaved file
  uint16_t VolSequenceLE;      //28..29 volume sequence number
  uint16_t VolSequenceBE;            //30..31
  uint8_t Len_Fi;          //32     N, the identifier length
  uint8_t FileName[512];      //33     identifier
...
struct iso_dirtree
{
  ...
  uint32_t Length;      // number of uint8_ts of file data or length of directory
...
public:
  ...
  uint32_t SetFilePointer(HANDLE hFile, long lDistanceToMove, long* lpDistanceToMoveHigh, uint32_t dwMoveMethod );
...
protected:
  ...
  std::string GetThinText(uint8_t* strTxt, int iLen );

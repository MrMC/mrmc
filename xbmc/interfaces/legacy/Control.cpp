  ...
  namespace xbmcgui
  {
    ...
    void ControlList::sendLabelBind(int tail)
    {
      ...
      if (CLiteUtils::IsLite())
        trimSize = CLiteUtils::GetItemSizeLimit();
      else
        trimSize = vecItems.size();
      // construct a CFileItemList to pass 'em on to the list
      CGUIListItemPtr items(new CFileItemList());
      for (unsigned int i = vecItems.size() - tail; i < vecItems.size() && i < trimSize; i++)
        ((CFileItemList*)items.get())->Add(vecItems[i]->item);

      if (CLiteUtils::IsLite())
      {
        int preTrimSize = vecItems.size();

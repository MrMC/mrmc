...
void XBPython::Announce(AnnouncementFlag flag, const char *sender, const char *message, const CVariant &data)
{
  ...
  std::string str;
  CJSONVariantWriter::Write(data, str, g_advancedSettings.m_jsonOutputCompact);
  
  OnNotification(sender, std::string(ANNOUNCEMENT::AnnouncementFlagToString(flag)) + "." + std::string(message), str);
}
...
bool XBPython::OnScriptInitialized(ILanguageInvoker *invoker)
{
  ...
  if (!m_bInitialized)
  {
    ...
    if (!CUtil::GetFrameworksPath(true).empty())
    {
      ...
      setenv("TEMP", CSpecialProtocol::TranslatePath("special://temp/temp").c_str(), 1);
      setenv("PYTHONHOME", CSpecialProtocol::TranslatePath("special://xbmc/system").c_str(), 1);
      setenv("PYTHONPATH", CSpecialProtocol::TranslatePath("special://xbmc/system").c_str(), 1);
      CLog::Log(LOGDEBUG, "TEMP -> %s", CSpecialProtocol::TranslatePath("special://temp/temp").c_str());

...
CAudioDecoder::CAudioDecoder()
{
  ...
  memset(&m_pcmInputBuffer, 0, INPUT_SIZE * sizeof(uint8_t));
...
void *CAudioDecoder::GetData(unsigned int samples)
{
  ...
  {
    CLog::Log(LOGWARNING, "CAudioDecoder::GetData() more bytes/samples (%i) requested than we have to give (%zu)!", size, m_pcmBuffer.getMaxReadSize());

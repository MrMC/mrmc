...
CDVDInputStreamNavigator::CDVDInputStreamNavigator(IDVDPlayer* player, const CFileItem& fileitem)
...
bool CDVDInputStreamNavigator::Open()
{
  ...
  int32_t readahead_flag = URIUtils::HasExtension(m_item.GetPath(), ".iso|.img") ? 0 : 1;
...
int CDVDInputStreamNavigator::Read(uint8_t* buf, int buf_size)
{
  ...
  while (true)
  {
    ...
    if (navresult == NAVRESULT_HOLD)
      ...
      return -1;
    else if (navresult == NAVRESULT_CSS)
    {
...
int CDVDInputStreamNavigator::ProcessBlock(uint8_t* dest_buffer, int* read)
{
  ...
  else if (result == DVDNAV_STATUS_CSS)
  {
    CLog::Log(LOGERROR,"CSS Error getting next block: %s\n", m_dll.dvdnav_err_to_string(m_dvdnav));
    m_bEOF = true;
    return NAVRESULT_CSS;
  }
...
bool CDVDInputStreamNavigator::GetSubtitleStreamInfo(const int iId, DVDNavStreamInfo &info)
{
  ...
  if( m_dll.dvdnav_get_spu_attr(m_dvdnav, streamId, &subp_attributes) == DVDNAV_STATUS_OK )
  {
    ...
    g_LangCodeExpander.ConvertToISO6392B(lang, info.language);
    return true;
  }
  return false;
}
...
void CDVDInputStreamNavigator::GetVideoResolution(uint32_t* width, uint32_t* height)
{
  ...
  m_dll.dvdnav_get_video_resolution(m_dvdnav, width, height);
}

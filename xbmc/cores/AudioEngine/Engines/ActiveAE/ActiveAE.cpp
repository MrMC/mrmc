...
void CActiveAE::ApplySettingsToFormat(AEAudioFormat &format, AudioSettings &settings, int *mode)
{
           ...
           (format.m_channelLayout.Count() > 2 || settings.stereoupmix))
  {
    ...
    format.m_streamInfo.m_channels = 2;
    format.m_streamInfo.m_sampleRate = 48000;
    format.m_streamInfo.m_frameSize = 2560;   // 640kbps
    if (mode)
      *mode = MODE_TRANSCODE;
  }
  else
     ...
     (m_mode != MODE_TRANSCODE || (m_encoderBuffers && !m_encoderBuffers->m_freeSamples.empty())))
  {
    ...
    for (it = m_streams.begin(); it != m_streams.end(); ++it)
    {
      ...
      if (buf->timestamp)
      {
        ...
        if (m_mode == MODE_RAW)
        {
          pts = buf->timestamp - (*it)->m_format.m_streamInfo.GetDuration();
        }
...
void CActiveAE::UnregisterAudioCallback()
{
  ...
  if (m_audioCallback.empty())
    return;
  // fake it until we change UnregisterAudioCallback
  IAudioCallback *pCallback = m_audioCallback.front();

...
#define IEC61937_TYPE_TRUEHD  0x16
...
static const uint8_t mat_end_code[24] = { 0xC3, 0xC2, 0xC0, 0xC4, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x97, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

void CBitstreamTrueHDMAT::MATWriteHeader()
{
  assert(m_bsOutput.size() == 0);

  int16_t dwSize = BURST_HEADER_SIZE + sizeof(mat_start_code);

  // reserve size for the IEC header and the MAT start code
  m_bsOutput.resize(dwSize);

  uint8_t *p = m_bsOutput.data();

  // IEC burst header
  AV_WB16(p + 0, IEC61937_PREAMBLE1);
  AV_WB16(p + 2, IEC61937_PREAMBLE2);
  AV_WB16(p + 4, IEC61937_TYPE_TRUEHD);
  AV_WB16(p + 6, 61424);
  ...
  // The MAT metadata counts as padding, if we're scheduled to write any, which mean the start int8_ts should reduce any further padding.
...
void CBitstreamTrueHDMAT::MATWritePadding()
{
  if (m_TrueHDMATState.padding > 0)
  {
    // allocate padding (on the stack of possible)
    uint8_t *padding = (uint8_t*)malloc(m_TrueHDMATState.padding);
...
void CBitstreamTrueHDMAT::MATAppendData(const uint8_t *p, int size)
{
  m_bsOutput.insert(m_bsOutput.end(), p, p + size);
  m_TrueHDMATState.mat_framesize += size;
}

int CBitstreamTrueHDMAT::MATFillDataBuffer(const uint8_t *p, int size, bool padding)
{
  if (m_bsOutput.size() >= MAT_BUFFER_LIMIT)
    return size;
  ...
  if (m_bsOutput.size() <= MAT_POS_MIDDLE && m_bsOutput.size() + size > MAT_POS_MIDDLE)
  {
    // write as much data before the middle code as we can
    int nint8_tsBefore = MAT_POS_MIDDLE - m_bsOutput.size();
    MATAppendData(p, nint8_tsBefore);
    remaining -= nint8_tsBefore;
    ...
    if (remaining > 0)
    {
      remaining = MATFillDataBuffer(p + nint8_tsBefore, remaining, padding);
    }
  ...
  if (m_bsOutput.size() + size >= MAT_BUFFER_LIMIT)
  {
    // write as much data before the middle code as we can
    int nint8_tsBefore = MAT_BUFFER_LIMIT - m_bsOutput.size();
    MATAppendData(p, nint8_tsBefore);
    remaining -= nint8_tsBefore;
    ...
    assert(m_bsOutput.size() == MAT_BUFFER_SIZE);
...
void CBitstreamTrueHDMAT::MATFlushPacket(unsigned int *dataSize, uint8_t *packedBuffer)
{
  if (m_bsOutput.size() > 0) {
    assert(m_bsOutput.size() == 61440);

    // Do not include IEC header
    *dataSize  = CAEPackIEC61937::PackTrueHD(m_bsOutput.data() + BURST_HEADER_SIZE, m_bsOutput.size() - BURST_HEADER_SIZE, packedBuffer);
    m_bsOutput.clear();
  }
}

bool CBitstreamTrueHDMAT::BitstreamTrueHD(const uint8_t *p, int buffsize, unsigned int *dataSize, uint8_t *packedBuffer)
{
  ...
  // In average all frames are always padded to 2560 int8_ts, so that 24 frames fit in one MAT frame, however
  ...
  // A constant padding to 2560 int8_ts can work (this is how the ffmpeg spdifenc module works), however
  ...
  while (m_TrueHDMATState.padding > 0)
  {
    ...
    assert(m_TrueHDMATState.padding == 0 || m_bsOutput.size() == MAT_BUFFER_SIZE);

    // Buffer is full, submit it
    if (m_bsOutput.size() == MAT_BUFFER_SIZE)
    {
      MATFlushPacket(dataSize, packedBuffer);
  ...
  if (remaining || m_bsOutput.size() == MAT_BUFFER_SIZE)
  {
    // flush out old data
    MATFlushPacket(dataSize, packedBuffer);

...
void CAdvancedSettings::Initialize()
{
  ...
  m_videoFpsDetect = 1;
  m_videoBusyDialogDelay_ms = 1000;
  ...
  m_moviesExcludeFromScanRegExps.push_back("_gsdata_");
  ...
  m_tvshowExcludeFromScanRegExps.push_back("_gsdata_");
...
void CAdvancedSettings::ParseSettingsFile(const std::string &file)
{
  ...
  if (pElement)
  {
    ...
    XMLUtils::GetInt(pElement, "busydialogdelayms", m_videoBusyDialogDelay_ms, 0, 4000);
...
void CAdvancedSettings::SettingOptionsLoggingComponentsFiller(const CSetting *setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
  ...
  list.push_back(std::make_pair(g_localizeStrings.Get(671), LOGWEBSERVER));
#ifdef HAS_DBUS
...
#endif
  list.push_back(std::make_pair(g_localizeStrings.Get(684), LOGDATABASE));
}

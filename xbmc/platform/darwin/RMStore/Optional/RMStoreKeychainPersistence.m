...
- (void)persistTransactionProductID:(NSString*)productIdentifier
{
  NSDictionary *transactions = [self transactionsDictionary];
  NSInteger count = [transactions[productIdentifier] integerValue];
  count++;
...
- (void)removeProductOfIdentifier:(NSString*)productIdentifier
{
  NSDictionary *transactions = [self transactionsDictionary];

  NSMutableDictionary *updatedTransactions = [NSMutableDictionary dictionaryWithDictionary:transactions];
  [updatedTransactions removeObjectForKey:productIdentifier];
  [self setTransactionsDictionary:updatedTransactions];
}
...
- (void)dumpProducts
{
    ...
    if ([productIdentifiers count] > 0)
    {
        ...
        {
            NSLog(@"Check purchases %@:", identifier);
        }
    }
    else
    {
        NSLog(@"Check purchases - no purchases");
    }
}
